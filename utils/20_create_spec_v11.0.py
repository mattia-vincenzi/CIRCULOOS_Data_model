...
def exist_page(url):
    ...
    output = []
    try:
        pointer = requests.get(url, timeout=1)
    ...
    if dataModel in filterDataModel or not allowFilterDataModel:
        ...
        for lang in languages:
            ...
            if 'customRepository' in globals():
                urlModelFile=customRepository+modelYaml
            else:
                urlModelFile = urlNotesRoot + "/" + repoName + "/" + dataModel + "/" + modelYaml
            print(urlModelFile)
            request = requests.get(urlModelFile, timeout=1)
            print("1. status code:" + str(request.status_code))
            ...
            if 'customRepository' in globals():
                schemaUrl=customRepository+modelYaml
            else:
                # this is retrieving the full model from the repo.
                schemaUrl = "https://smart-data-models.github.io/" + repoName + "/" + dataModel + "/" + modelYaml
            print(schemaUrl)
            request = requests.get(schemaUrl, timeout=1)
            print("2. status code:" + str(request.status_code))
            if request.status_code == 200:
            ...
            if 'customRepository' in globals():
                urlNotesFile=customRepository+"notes.yaml"
            else:
                urlNotesFile = urlNotesRoot + "/" + repoName + "/" + dataModel + "/notes.yaml"
            print("urlNotesFile" + "--")
            print(urlNotesFile)
            request = requests.get(urlNotesFile, timeout=1)
            print("3. status code:" + str(request.status_code))
            if request.status_code == 200:
                print("Reading notes")
                notes = read_yaml(urlNotesFile)
                print(notes)
            else:
                print("3. Not found "+ urlNotesFile)
                notes = {}
            try:
            ...
            if pointer.status_code == 200:
                ...
                print("4. Not found fullModel.yaml")
            ...
            for exampleType in namesExamples:
                if 'customRepository' in globals():
                    exampleUrl=customRepository+ "examples/" + namesExamples[exampleType]
                else:
                ...
                if exist_page(exampleUrl)[0]:
                    ...
                    if 'customRepository' in globals():
                        urlExampleFile=customRepository+"examples/" + namesExamples[exampleType]
                    else:
                        urlExampleFile = urlNotesRoot + "/" + repoName + "/" + dataModel + "/examples/" + namesExamples[exampleType]
                    request = requests.get(urlExampleFile, timeout=1)
                    print("5. status code:" + str(request.status_code))
                    if request.status_code == 200:
                        print("Reading model")
            ...
            if 'customRepository' in globals():
                with open(specFileName, 'w') as file:
                    file.write(specContentReviewed)
            else:
